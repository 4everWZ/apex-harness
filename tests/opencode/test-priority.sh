...
# 1. Create in apex location (lowest priority)
mkdir -p "$APEX_SKILLS_DIR/priority-test"
cat > "$APEX_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: APEX version of priority test skill
---
# Priority Test Skill (APEX Version)

This is the APEX version of the priority test skill.

PRIORITY_MARKER_APEX_VERSION
EOF
...
if [ -f "$APEX_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] APEX version exists"
else
    echo "  [FAIL] APEX version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled apex priority
echo ""
echo "Test 2: Documenting personal vs apex priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled apex version instead of personal version"

# Test 3: Document project vs bundled apex priority
echo ""
echo "Test 3: Documenting project vs personal/apex priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled apex version instead of project version"

# Test 4: Test a non-colliding bundled apex skill is still available
echo ""
echo "Test 4: Testing non-colliding apex skill remains available..."

mkdir -p "$APEX_SKILLS_DIR/apex-only-test"
cat > "$APEX_SKILLS_DIR/apex-only-test/SKILL.md" <<'EOF'
---
name: apex-only-test
description: APEX-only priority test skill
---
# APEX Only Test Skill

PRIORITY_MARKER_APEX_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"apex-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_APEX_ONLY_VERSION" "Non-colliding apex skill is still registered"
