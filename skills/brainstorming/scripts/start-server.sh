...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.agent-work/brainstorm/
...
if [[ -n "$PROJECT_DIR" ]]; then
  SESSION_DIR="${PROJECT_DIR}/.agent-work/brainstorm/${SESSION_ID}"
  # Persist the bound port and key per project so a restart reuses them and an
  # already-open browser tab reconnects to the same URL with a valid cookie.
  export BRAINSTORM_PORT_FILE="${PROJECT_DIR}/.agent-work/brainstorm/.last-port"
  export BRAINSTORM_TOKEN_FILE="${PROJECT_DIR}/.agent-work/brainstorm/.last-token"
else
  SESSION_DIR="/tmp/brainstorm-${SESSION_ID}"
fi

if [[ -e "$SESSION_DIR" ]]; then
  echo '{"error":"refusing pre-existing brainstorm session directory"}' >&2
  exit 1
fi
