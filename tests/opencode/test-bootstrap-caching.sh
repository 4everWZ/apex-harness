...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$APEX_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$APEX_SKILLS_DIR/using-apex/SKILL.md" "$TEST_HOME/using-apex.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$APEX_PLUGIN_FILE" missing
}
