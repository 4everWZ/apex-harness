# Repository rules

- This repository targets Codex only.
- `skills/` contains exactly `apex-harness` and `managing-project-docs`.
- Each skill has its own frontmatter trigger and neither requires a router.
- Execution and documentation responsibilities must not overlap.
- Preserve existing repository documentation conventions before applying fallback paths.
- Do not add routers, aliases, hooks, runtime adapters, or duplicate workflows.
- Keep fallback templates under `skills/managing-project-docs/assets/templates/`.
- Static validation must not be described as behavioral evaluation.
- Keep static validation structural; do not encode preferred wording, headings, metadata, or workflow behavior as validation rules.
