...
const extensionPath = resolve(repoRoot, '.pi/extensions/apex.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-apex/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'apex');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/apex.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /APEX governs this skill collection/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /APEX governs this skill collection/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /APEX governs this skill collection/);
