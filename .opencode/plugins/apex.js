/**
 * APEX plugin for OpenCode.ai
 *
 * Injects the APEX bootstrap context via message transform.
...
export const ApexPlugin = async ({ client, directory }) => {
  const homeDir = os.homedir();
  const apexSkillsDir = path.resolve(__dirname, '../../skills');
  ...
  const getBootstrapContent = () => {
    ...
    // Try to load the using-apex entry skill
    const skillPath = path.join(apexSkillsDir, 'using-apex', 'SKILL.md');
...
${toolMapping}
  ...
  return {
    // Inject skills path into live config so OpenCode discovers APEX skills
    ...
    config: async (config) => {
      ...
      if (!config.skills.paths.includes(apexSkillsDir)) {
        config.skills.paths.push(apexSkillsDir);
      }
    },
