...
start_owned_process() {
  local server_id="$1"
  bash -c 'while :; do sleep 60; done' brainstorm-server \
    "--brainstorm-server-id=$server_id" > /dev/null 2>&1 &
  SRV=$!
}
...
# --- Test 2a: ps fallback recognizes the exact instance id ---
SESS="$(mktemp -d)"; track_dir "$SESS"; mkdir -p "$SESS/content" "$SESS/state"
SERVER_ID="$(new_server_id)"
printf '%s\n' "$SERVER_ID" > "$SESS/state/server-instance-id"
start_owned_process "$SERVER_ID"
...
disown "$SRV" 2>/dev/null || true
echo "$SRV" > "$SESS/state/server.pid"
OUT="$(BRAINSTORM_PROC_ROOT=/definitely-missing "$STOP" "$SESS")"
sleep 0.3
if kill -0 "$SRV" 2>/dev/null; then
  bad "ps fallback leaves owned server running" "$OUT"
else
  wait "$SRV" 2>/dev/null || true
  untrack_pid "$SRV"
  case "$OUT" in
    *stopped*) ok "ps fallback stops the exact owned instance" ;;
    *) bad "ps fallback stopped process but status was unexpected" "$OUT" ;;
  esac
fi
