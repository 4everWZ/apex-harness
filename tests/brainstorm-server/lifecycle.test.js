...
function pathForBash(file) {
  const shellDir = execFileSync('bash', ['-lc', 'pwd'], {
    cwd: path.dirname(file),
    encoding: 'utf8'
  }).trim();
  return `${shellDir}/${path.basename(file)}`;
}

function bashHasNode() {
  try {
    execFileSync('bash', ['-lc', 'command -v node >/dev/null 2>&1']);
    return true;
  } catch {
    return false;
  }
...
function newestSessionDir(projectDir) {
  const sessionDir = execFileSync('bash', [
    '-lc',
    'find "$1/.agent-work/brainstorm" -mindepth 1 -maxdepth 1 -type d -print | sort | tail -1',
    'bash',
    projectDir
  ], { encoding: 'utf8' }).trim();
  assert(sessionDir, `expected at least one session dir under ${projectDir}/.agent-work/brainstorm`);
  return sessionDir;
}
...
async function runTests() {
  ...
  async function test(name, fn) {
    try { await fn(); console.log(`  PASS: ${name}`); passed++; }
      ...
      if (e && e.skip) { console.log(`  SKIP: ${name} (${e.message})`); skipped++; }
      else { console.log(`  FAIL: ${name}`); console.log(`    ${e.message}`); failed++; }
    }
  }
  ...
  await test('start-server.sh --idle-timeout-minutes sets the timeout', async () => {
    if (!bashHasNode()) {
      const error = new Error('node is unavailable inside bash');
      error.skip = true;
      throw error;
    }
    ...
    try {
      if (isWindowsLikeShell()) {
        startProcess = spawn('bash', [pathForBash(START), '--project-dir', dir, '--idle-timeout-minutes', '5']);
        info = firstServerStarted(await waitForStartedOutput(startProcess));
      } else {
        const out = execFileSync('bash', [pathForBash(START), '--project-dir', dir, '--idle-timeout-minutes', '5', '--background'], { encoding: 'utf8' });
      ...
      if (sessionDir) execFileSync('bash', [pathForBash(STOP), sessionDir], { stdio: 'ignore' });
