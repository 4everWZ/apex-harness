#!/usr/bin/env bash
# Regression check: worktree location is not treated as ownership provenance.
...
assert_not_contains "$FINISHING_SKILL" '`.worktrees/` or `worktrees/`' "finishing does not infer ownership from path"
assert_contains "$FINISHING_SKILL" "recorded agent-creation provenance" "finishing requires explicit workspace provenance"
assert_contains "$FINISHING_SKILL" "separate explicit request" "merge does not imply cleanup"
