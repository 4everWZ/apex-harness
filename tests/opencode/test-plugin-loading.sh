#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the apex plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/apex.js"
...
skill_count=$(find "$APEX_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $APEX_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-apex skill exists (critical for bootstrap)
echo "Test 3: Checking using-apex skill (required for bootstrap)..."
if [ -f "$APEX_SKILLS_DIR/using-apex/SKILL.md" ]; then
    echo "  [PASS] using-apex skill exists"
else
    echo "  [FAIL] using-apex skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$APEX_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/apex/' "$APEX_PLUGIN_FILE"; then
