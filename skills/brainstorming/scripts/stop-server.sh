...
# under /tmp (ephemeral). Persistent directories (.agent-work/) are
...
if ! SESSION_DIR="$(realpath -e -- "$SESSION_DIR" 2>/dev/null)"; then
  echo '{"error": "session_dir must resolve to an existing directory"}'
  exit 1
fi

EPHEMERAL_SESSION=false
if [[ "$(dirname -- "$SESSION_DIR")" == "/tmp" ]] &&
   [[ "$(basename -- "$SESSION_DIR")" =~ ^brainstorm-[0-9]+-[0-9]+$ ]]; then
...
command_line_for_pid() {
  local pid="$1"
  local proc_root="${BRAINSTORM_PROC_ROOT:-/proc}"
...
if [[ -f "$PID_FILE" ]]; then
  ...
  # Delete only a canonical immediate /tmp session created by start-server.sh.
