...
`.../plugins/apex/skills/<skill-name>/SKILL.md` with `IsSkillFile: true`.
...
Skills dispatch a bounded inline prompt. On Antigravity:

| Skill dispatch form | Antigravity equivalent |
|---------------------|----------------------|
| An implementer-style bounded prompt (writes code, runs tests) | Pass it to `invoke_subagent` with `TypeName: "self"` |
